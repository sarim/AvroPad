...
SuggestionBuilder.prototype = {
    ...
    _loadCandidateSelectionsFromFile: function(){
        try {
			if (localStorage.candidate_selections)
			{
				this._candidateSelections = JSON.parse(localStorage.candidate_selections);
    ...
    _saveCandidateSelectionsToFile: function(){
        try {
			...
			localStorage.candidate_selections = JSON.stringify(this._candidateSelections);
            /*
			var file = gio.File.new_for_path ( GLib.get_home_dir() + "/.candidate-selections.json");
