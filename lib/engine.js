...
(function($){
    ...
    $.fn.bnKb = function(options){
        ...
        writingMode = "b";
        carry = "";
        $(this).unbind("keypress keydown keyup cmttxt");
        
        $(this).keyup($.fn.bnKb.ku);
        $(this).keydown($.fn.bnKb.kd);
        $(this).keypress($.fn.bnKb.kp);
		$(this).bind("cmttxt",$.fn.bnKb.committext);
    ...
    $.fn.bnKb.committext = function(ev,bntext,banglish){
		
		
		$.fn.bnKb.iac(this, bntext, 1);
		carry = "";
		
		suggestionBuilder.updateCandidateSelection(banglish,bntext);	
    }
    ...
    $.fn.bnKb.kp = function(ev){
        ...
        if (ctrlPressed) 
            $("#status").html("Not Processing");
        else {
            var _carry = carry;
            carry += keystring;
            //processing intellisense
            if (opts.driver.supportIntellisense) {
                var mods = opts.driver.intellisense(keystring, _carry,ev);
                if (mods) {
                    keystring = mods.keystring
                    carry = mods.carry;
					mtype = 0;
					if (mods.type){
						lastInsertedString = mods.lastInsertedString;
						mtype = mods.type;
					}
					
		            $.fn.bnKb.iac(this, mods.replacement, mtype);
		            ev.stopPropagation();
		            return false;
                }
            }
    ...
    $.fn.bnKb.kd = function(ev){
		...
		if (keycode == 8){
			keystring = "";
			var _carry = carry.substr(0,carry.length -1 );
            var mods = opts.driver.intellisense(keystring, _carry,ev);
            if (mods) {
                keystring = mods.keystring
                carry = mods.carry;
